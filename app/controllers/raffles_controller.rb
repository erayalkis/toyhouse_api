class RafflesController < ApplicationController
  ...
  def calculate_tickets
    ...
    favorites[:favorites].each do |favorite| 
      ...
      tickets[username] = {}
      tickets[username][:ticket_count] = params[:fav_ticket_count].to_i || 1
      tickets[username][:image] = favorite[:image]
    end

    comments[:comments].each do |comment|
      username = comment[:user][:username]
      tickets[username][:ticket_count] += params[:comment_ticket_count].to_i || 1 if tickets.has_key?(username)
    end
    
    subscribers[:subscribers].each do |subscriber|
      username = subscriber[:username]
      tickets[username][:ticket_count] += params[:subscribe_ticket_count].to_i || 1 if tickets.has_key?(username) 
    end
